...
export function ColorPicker({
  ...
  return (
    <div className="w-full max-w-[320px] space-y-3 rounded-xl border bg-background p-3 shadow-sm">
      <div className="relative h-56 w-full overflow-hidden rounded-lg" style={checkerboard}>
        <div
      ...
      <div className="space-y-2">
        <div className="relative h-4 overflow-hidden rounded-full" style={checkerboard}>
          <input
            type="range"
            min={0}
            max={360}
      ...
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-2">
        ...
        <Input
          ...
          onChange={(event) => {
            ...
            const parts = raw
              .replace(/%/g, '')
              .split(',')
              .map((part: any) => Number(part.trim()))
            if (parts.length !== 3 || parts.some((value: any) => Number.isNaN(value))) return
        ...
        <Select value={format} onValueChange={(value) => setFormat(value as PickerFormat)}>
          <SelectTrigger className="w-[78px] rounded-full border-none">
            <SelectValue />
          </SelectTrigger>
